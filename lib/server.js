    ...
    start() {
        ...
        table.filter(routes => routes.path === "/api/delegates/{id}")[0].settings.handler = request => {
            ...
            if (database.delegates) {
                delegate = database.delegates.findById(request.params.id);
            } else {
                const { Database } = require("@arkecosystem/core-interfaces");
                delegate = database.wallets.findById(Database.SearchScope.Delegates, request.params.id);
            }
            if (!delegate) {
        ...
        table.filter(routes => routes.path === "/api/delegates")[0].settings.handler = request => {
            let delegates;
            if (database.delegates) {
                delegates = database.delegates.search({
                    ...request.query,
                   ...paginate(request)
                });
            } else {
                const { Database } = require("@arkecosystem/core-interfaces");
                delegates = database.wallets.search(Database.SearchScope.Delegates, {
                    ...request.query,
                   ...paginate(request)
                });
            }
        ...
        const missed = (type, request) => {
            ...
            const rows = this.getMissed(type, delegate.username ? delegate.username : delegate.attributes.delegate.username);
